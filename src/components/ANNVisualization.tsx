...
export function ANNVisualization() {
    ...
    // --- Geometry calculation for clipping lines ---
    const getRegionBoundaries = (nodePath: string) => {
        let bounds = [
    ...
    const { currentSplit, splitsToDraw, activeSearchPath, finalLeaf, regions } = useMemo(() => {
        ...
        const splits = step > 0 ? splitSequence.slice(0, Math.min(step, splitSequence.length)) : [];
        let searchPath = '';
        let finalNode: TreeNode | undefined;
        let visibleRegions: { points: Point[], color: string, boundary: {x:number, y:number}[] }[] = [];
        ...
        if (step > splitSequence.length) {
            ...
            visibleRegions = leaves.map(leaf => ({
                ...
                boundary: getRegionBoundaries(leaf.path)
            }));
        }
