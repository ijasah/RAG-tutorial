...
import { HelpCircle, FileText, Bot, Search, RefreshCw, ArrowRight, MessageSquare, Sparkles } from 'lucide-react';
...
const FlowNode = ({ icon, title, children, status, step, currentStep }: { icon: React.ReactNode, title: string, children: React.ReactNode, status: 'inactive' | 'active' | 'complete', step: number, currentStep: number }) => (
    <motion.div
        ...
        transition={{ duration: 0.4, delay: 0.1 * (step -1) }}
        className={`relative p-4 border rounded-lg transition-all duration-300 ${status === 'active' ? 'border-primary bg-primary/10' : 'border-border bg-muted/40'}`}
    >
        <div className="flex items-center gap-3 mb-2">
            <span className={`flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold transition-all duration-300 shrink-0 ${status === 'inactive' ? 'bg-muted border' : 'bg-primary text-primary-foreground'}`}>
                {step}
            </span>
    ...
    return (
        <Card className="bg-card/60 border-primary/20">
            <CardHeader>
                ...
                <CardDescription className="pt-2 h-12 flex items-center">
                     {getStepDescription()}
                </CardDescription>
            </CardHeader>
                ...
                <div className="space-y-2">
                    ...
                    <FlowNode icon={<FileText />} title="Retrieve Top K Chunks" status={getStatus(4)} step={4} currentStep={step}>
                        <div className="space-y-1">
                             <p className="p-2 border rounded bg-background flex items-center gap-2"><Badge>Chunk 1</Badge> RAG grounds LLMs in external knowledge, which reduces factual inaccuracies (hallucinations).</p>
                             <p className="p-2 border rounded bg-background flex items-center gap-2"><Badge>Chunk 2</Badge> By providing up-to-date context, RAG improves the accuracy and relevance of generated answers.</p>
                        </div>
                    </FlowNode>
